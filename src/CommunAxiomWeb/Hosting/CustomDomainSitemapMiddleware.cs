...
namespace CommunAxiomWeb.Hosting
{
    ...
    /// </summary>
    public class CustomDomainSitemapMiddleware : MiddlewareBase
    {
        ...
        public CustomDomainSitemapMiddleware(RequestDelegate next, IOptions<RoutingOptions> options, IConfiguration configuration, ILoggerFactory factory = null) : base(next, factory)
        {
        ...
        public override async Task Invoke(HttpContext context, IApi api, IApplicationService service)
        {
            if (_options.UseSitemapRouting && !IsHandled(context) && !context.Request.Path.Value.StartsWith("/manager/assets/"))
            {
                ...
                var host = _configuration["HostOverride"] ?? context.Request.Host.Host;
                var scheme = _configuration["SchemeOverride"] ?? context.Request.Scheme;
